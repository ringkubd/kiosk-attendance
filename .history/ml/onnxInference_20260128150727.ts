...
export async function initializeModel(): Promise<void> {
  try {
    ...
    // Load model asset using direct require - use try-catch for better error reporting
    let asset: any;
    try {
      const modelModule = require("../assets/models/MobileFaceNet.onnx");
      asset = Asset.fromModule(modelModule);
    ...
    if (!modelUri) {
      throw new Error("Failed to get URI for model asset after download");
    }

    if (modelUri.startsWith("asset:/")) {
      const cachePath = `${FileSystem.cacheDirectory}MobileFaceNet.onnx`;
      const cached = await FileSystem.getInfoAsync(cachePath);
      if (!cached.exists) {
        Logger.info(`Caching model to: ${cachePath}`);
        await FileSystem.downloadAsync(modelUri, cachePath);
      }
      modelUri = cachePath;
    }
    ...
    Logger.info(`Creating ONNX session from: ${modelUri}`);
