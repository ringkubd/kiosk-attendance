...
function decodeBase64ToImageData(base64: string): ImageData {
  try {
    ...
    if (decoded.width === 0 || decoded.height === 0) {
      throw new Error(`Invalid image dimensions: ${decoded.width}x${decoded.height}`);
    }

    logger.debug(`Decoded image: ${decoded.width}x${decoded.height}, data size: ${decoded.data.length}`);
    ...
    throw error; // Re-throw instead of silently returning blank tensor
  }
}
