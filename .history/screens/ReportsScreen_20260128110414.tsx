...
export default function ReportsScreen() {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      // Save to file (guarded for TS typings) with robust fallbacks
      const fs = FileSystem as any;
      let fileUri: string | null = null;

      // Try preferred directories: documentDirectory, then cacheDirectory
      const candidates = [fs.documentDirectory, fs.cacheDirectory];
      let baseDir = candidates.find((d) => !!d) ?? "";

      try {
        if (!baseDir) {
          // No well-known base; try cacheDirectory directly
        ...
        // Last-ditch fallback: try writing to cacheDirectory root if available
        if (!fileUri && fs.cacheDirectory) {
          try {
            fileUri = `${fs.cacheDirectory}attendance_report_${Date.now()}.csv`;
            await FileSystem.writeAsStringAsync(fileUri, csvContent);
          } catch {
            // rethrow the original meaningful error
            throw writeErr;
          }
        } else {
          throw writeErr;
      ...
      if (fileUri && (await Sharing.isAvailableAsync())) {
        ...
        throw new Error("Failed to write report file to any available directory");
      }
    } catch (err: any) {
