...
export default function ReportsScreen() {
  ...
  // Group logs by employee and date to determine daily status
  const getEmployeeDayStatus = (empId: string, dayStart: number) => {
    const dayEnd = dayStart + 24 * 60 * 60 * 1000;
    const dayLogs = logs.filter(
      (log) =>
        log.employee_id === empId &&
        log.ts_local >= dayStart &&
        log.ts_local < dayEnd,
    );

    if (dayLogs.length === 0) return { label: "ABSENT", tone: "error" };

    const hasIn = dayLogs.some((log) => log.type === "IN");
    const hasOut = dayLogs.some((log) => log.type === "OUT");

    if (!hasIn) return { label: "ABSENT", tone: "error" };
    if (hasIn && !hasOut) return { label: "NO LOGOUT", tone: "warning" };
    if (hasIn && hasOut) return { label: "PRESENT", tone: "success" };

    return { label: "PRESENT", tone: "success" };
  };

  const renderRow = (item: AttendanceLogWithEmployee) => {
    const dayStart = Math.floor(item.ts_local / (24 * 60 * 60 * 1000)) * 24 * 60 * 60 * 1000;
    const status = getEmployeeDayStatus(item.employee_id, dayStart);
