...
export async function initializeSettings(): Promise<void> {
  try {
    ...
    if (!syncEnabled) {
      await SecureStore.setItemAsync(KEYS.SYNC_ENABLED, "false");
    }
    ...
    const activeBranchId = await SecureStore.getItemAsync(KEYS.ACTIVE_BRANCH_ID);
    if (!activeBranchId) {
...
export async function setActiveOrgBranchIds(
  ...
  await SecureStore.setItemAsync(KEYS.ACTIVE_BRANCH_ID, branchId);
